...
import { default as downloadContentFromMessage } from "@xyzendev/baileys"
...
export const totalcase = () => {
   var file = fs.readFileSync("../core.js").toString()
...
export const buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./RellMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./RellMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./RellMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./RellMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./RellMedia/video/${filename}.mp4`), unlink(`./RellMedia/gif/${filename}.gif`)])
	return buffer5
				   }
 ...
 * @param {store} store 
 */
export const smsg = (rell, m, store) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = rell.decodeJid(m.fromMe && rell.user.id || m.participant || m.key.participant || m.chat || '');
    if (m.isGroup) m.participant = rell.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    m.mtype = getContentType(m.message);
    m.msg = (m.mtype == 'viewOnceMessage' ? m.message[m.mtype].message[getContentType(m.message[m.mtype].message)] : m.message[m.mtype]);

    // Ensure m.msg is defined before accessing its properties
    m.body = m.message.conversation || (m.msg && (m.msg.caption || m.msg.selectedId || m.msg.text)) || 
             ...
             (m.mtype == 'viewOnceMessage' && m.msg && m.msg.caption) || m.text;
    ...
    if (m.quoted) {
        ...
        m.quoted.sender = rell.decodeJid(m.msg.contextInfo.participant);
        m.quoted.fromMe = m.quoted.sender === rell.decodeJid(rell.user.id);
        ...
        m.quoted.delete = () => rell.sendMessage(m.quoted.chat, { delete: vM.key });
        m.quoted.copyNForward = (jid, forceForward = false, options = {}) => rell.copyNForward(jid, vM, forceForward, options);
        m.quoted.download = () => rell.downloadMediaMessage(m.quoted);
    }
}
  if (m.msg.url) m.download = () => rell.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? rell.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : rell.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => rell.copyNForward(jid, m, forceForward, options);
  rell.appenTextMessage = async (text, chatUpdate) => {
...
export const GIFBufferToVideoBuffer = async (image) => {
    ...
    var buffer5 = await fs.readFileSync(`./RellMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./RellMedia/trash/${filename}.mp4`), unlink(`./RellMedia/trash/${filename}.gif`)])
    return buffer5
}